...
SampleCommunication::sayPlayers(PlayerAgent *agent) {
    ...
    if (candidates.size() >= 3
        ...
        AbstractPlayerCont::const_iterator second = first;
        ++second;
        int second_unum = ((*second)->side() == wm.ourSide()
                           ? (*second)->unum()
                           : (*second)->unum() + 11);
        AbstractPlayerCont::const_iterator third = second;
        ++third;
...
SampleCommunication::sayTwoOpponents(PlayerAgent *agent) {
    ...
    std::vector<const PlayerObject *>::const_iterator second = first;
    ++second;
...
SampleCommunication::sayThreeOpponents(PlayerAgent *agent) {
    ...
    std::vector<const PlayerObject *>::const_iterator second = first;
    ++second;
    std::vector<const PlayerObject *>::const_iterator third = second;
    ++third;
